...
import services.dao.test.model.TestDbtest;
...
public class TestDbtestDAO {
	static ConnectionPool pool = ConnectionPool.getInstance();
	static TestDbtestDAO instance = null;

	public static TestDbtestDAO getInstance() {
		if (null == instance) {
			instance = new TestDbtestDAO("test");
		}
		return instance;
	}
	
	public static TestDbtestDAO getNewInstance(String dbName) {
		return new TestDbtestDAO(dbName);
	}
	
	private final String dbName;
	private TestDbtestDAO(String dbName) {
		this.dbName = dbName;
	}
	
	public void truncate() {
		String sql = String.format("TRUNCATE %s.dbtest", dbName);
	...
	public int insertReturnId(TestDbtest obj) {
		String sql = String.format("INSERT INTO %s.dbtest(id,value) VALUES ", dbName);
	...
	public void insert(TestDbtest obj) {
		String sql = String.format("INSERT INTO %s.dbtest(id,value) VALUES (?,?)", dbName);
	 ...
	 */
	@Deprecated
	public void insertBatch(List<TestDbtest> objList) {
		String sql = String.format("INSERT INTO %s.dbtest(id,value) VALUES (?,?)", dbName);
	...
	public void insertMultiple(List<TestDbtest> objList) {
		if (objList.size() == 0)
			return;
		StringBuilder sqlBuilder = new StringBuilder(String.format("INSERT INTO %s.dbtest(id,value) VALUES ", dbName));
	...
	public void insertMultipleBatch(List<TestDbtest> objList, int multipleSize) {
		if (objList.size() == 0)
			return;
		String sqlHead = String.format("INSERT INTO %s.dbtest(id,value) VALUES ", dbName);
		...
		try {
			...
			while (startIndex + multipleSize <= objList.size()) {
				int indexCount = 0;
				for (TestDbtest obj : objList.subList(startIndex, startIndex + multipleSize)) {
					indexCount = _constructPS(ps, obj, indexCount);
				}
				ps.addBatch();
	 ...
	 */
	@Deprecated
	public void insertBatchWithLimitedWindow(List<TestDbtest> objList, int windowSize) {
		int startIndex = 0;
		...
		while (startIndex < objList.size()) {
			...
			if (++displayCounter % 20 == 0)
				System.out.println(String.format("[inserting TestDbtest] %d / %d", startIndex, objList.size()));
		}
	}

	@Deprecated
	public void insertMultipleWithLimitedWindow(List<TestDbtest> objList, int windowSize) {
		int startIndex = 0;
		int displayCounter = 0;
		while (startIndex < objList.size()) {
	...
	public void insertMultipleBatchWithLimitedWindow(List<TestDbtest> objList, int batchWindowSize, int batchMount) {
		int startIndex = 0;
	...
	public void update(TestDbtest obj) {
		String sql = String.format("UPDATE %s.dbtest SET id = ?,value = ? WHERE id = ?", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		try {
	...
	public void updateBatch(List<TestDbtest> objList) {
		String sql = String.format("UPDATE %s.dbtest SET id = ?,value = ? WHERE id = ?", dbName);
		...
		try {
			...
			for (TestDbtest obj : objList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch updating TestDbtest] %d / %d", count, objList.size()));
				int indexCount = _constructPS(ps, obj, 0);
				ps.setInt(++indexCount, obj.getId());
				ps.addBatch();
			}
	...
	public void updateBatchWithIntegerFieldSelection(String whereField, Integer whereParamCount, List<List<Integer>> whereInValueList, //
		...
		StringBuilder sqlBuilder = new StringBuilder( //
			String.format("UPDATE %s.dbtest SET %s = ? WHERE %s IN (", dbName, setField, whereField));
	...
	public void delete(Integer id) {
		String sql = String.format("DELETE FROM %s.dbtest WHERE id = ?", dbName);
	...
	public void deleteBatch(List<Integer> idList) {
		...
		try {
			...
			for (Integer id : idList) {
				if(++count % 5000 == 0) 
					System.out.println(String.format("[batch deleting TestDbtest] %d / %d", count, idList.size()));
				ps.setInt(1, id);
				ps.addBatch();
			}
			ps.executeBatch();
	...
	public void deleteMultipleById(List<Integer> idList) {
		...
		StringBuilder sqlBuilder = new StringBuilder("DELETE FROM %s.dbtest WHERE id IN (");
	...
	public Integer selectMaxId() {
		Integer result = null;
		String sql = String.format("SELECT MAX(id) FROM %s.dbtest", dbName);
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public TestDbtest selectById(Integer id) {
		if (null == id) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.dbtest WHERE id = ?", dbName);
		TestDbtest result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<TestDbtest> selectByIdList(List<Integer> idList) {
		if (null == idList) {
			return null;
		} else if (idList.size() == 0) {
			return new ArrayList<TestDbtest>();
		}

		StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM %s.dbtest WHERE id IN (");
		for (int i = 0; i < idList.size(); i++) {
			sqlBuilder.append('?').append(',');
		}
		sqlBuilder.setCharAt(sqlBuilder.length() - 1, ')');
		...
		List<TestDbtest> result = new ArrayList<TestDbtest>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<TestDbtest> selectByIdListWithLimitedWindow(List<Integer> idList, Integer windowSize) {
		if (null == idList)
			return null;
	...
	public TestDbtest selectSingleByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.dbtest WHERE %s = ? LIMIT 1", dbName, field);
		TestDbtest result = null;
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<TestDbtest> selectByStringField(String field, String value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.dbtest WHERE %s = ?", dbName, field);
		List<TestDbtest> result = new ArrayList<TestDbtest>();
		DBConnection conn = pool.getConnection();
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<TestDbtest> selectByMultipleStringField(String field, List<String> valueList) {
		if (null == valueList) {
			return null;
		...
		StringBuilder sqlBuilder = new StringBuilder(String.format("SELECT * FROM %s.dbtest WHERE %s IN (", dbName, field));
	...
	public List<TestDbtest> selectByMultipleStringFieldWithLimitedWindow(String field, List<String> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public TestDbtest selectSingleByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.dbtest WHERE %s = ? LIMIT 1", dbName, field);
	...
	public List<TestDbtest> selectByIntegerField(String field, Integer value) {
		if (field == null || "".equals(field.trim())) {
			return null;
		}
		String sql = String.format("SELECT * FROM %s.dbtest WHERE %s = ?", dbName, field);
	...
	public List<TestDbtest> selectByMultipleIntegerField(String field, List<Integer> valueList) {
		if (null == valueList) {
			return null;
		} else if (valueList.size() == 0) {
			return new ArrayList<TestDbtest>();
	...
	public List<TestDbtest> selectByMultipleIntegerFieldWithLimitedWindow(String field, List<Integer> valueList, Integer windowSize) {
		if (null == valueList)
			return null;
	...
	public List<TestDbtest> walk(List<Field> fieldList, int start, int limit) {
		List<TestDbtest> result = new ArrayList<TestDbtest>();
		...
		String sql = String.format("SELECT %s FROM %s.dbtest WHERE id >= ? ORDER BY id LIMIT ?", fieldString, dbName);
		...
		try {
			...
			while (rs.next()) {
				if(defaultField)
					...
					TestDbtest obj = SqlConstructUtil._constructResult(fieldList, TestDbtest.class, rs);
	...
	public List<TestDbtest> walk(int start, int limit) {
		return walk(null, start, limit);
	}

	public List<TestDbtest> walkAll() {
		return walkAll(null, -1);
	}

	public List<TestDbtest> walkAll(int walkstep) {
	...
	public List<TestDbtest> walkAll(List<Field> fieldList, int walkstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		List<TestDbtest> result = new ArrayList<TestDbtest>();
		int startid = 0;
		int limit = 500;
		while (true) {
			List<TestDbtest> walk = walk(startid, limit);
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Loading TestDbtest] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public void fetchAll(Callback<TestDbtest> callback) {
		fetchAll(callback, -1);
	}

	public void fetchAll(Callback<TestDbtest> callback, int fetchstep) {
		fetchAll(callback, null, fetchstep);
	}
	
	public void fetchAll(Callback<TestDbtest> callback, List<Field> fieldList) {
		fetchAll(callback, fieldList, -1);
	}
	
	public void fetchAll(Callback<TestDbtest> callback, List<Field> fieldList, int fetchstep) {
		long t0 = System.currentTimeMillis();
		int stepCount = 0;
		int startid = 0;
		int limit = 500;
		while (true) {
			List<TestDbtest> walk = walk(fieldList, startid, limit);
			if (null == walk || walk.size() == 0)
				break;
			...
			stepCount++;
			if (stepCount % 20 == 0) {
				System.out.println(String.format("[Fetching TestDbtest] id:%d, timeUsed:%dms", startid, (System.currentTimeMillis() - t0)));
			}
	...
	public List<Field> _getFieldList(String[] fieldNameArray) throws NoSuchFieldException {
		...
		try {
			for (String fieldName : fieldNameArray) {
				result.add(TestDbtest.class.getDeclaredField(fieldName));
			}
	...
	public int _constructPS(PreparedStatement ps, TestDbtest obj, int indexCount) throws SQLException {
		SqlConstructUtil.__safeSetInt(ps, ++indexCount, obj.getId());
		SqlConstructUtil.__safeSetInt(ps, ++indexCount, obj.getValue());

		return indexCount;
	}

	public TestDbtest _constructResult(ResultSet rs) throws SQLException {
		TestDbtest obj = new TestDbtest();
		obj.setId(rs.getInt("id"));
